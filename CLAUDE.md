...
- **Path A session (2026-04-05): Seed appellation_rules from legal sources.** Provenance columns added (source_url, source_organization, source_document_title, source_accessed_date, source_text_excerpt, last_verified_at) to `appellation_rules` and `appellation_grapes`. **59 appellation_rules seeded** (0 → 59), all with 100% provenance coverage traceable to legal documents. Source breakdown: **36 INAO cahiers des charges** (French AOCs) + **10 MAPA pliegos de condiciones** (Spanish DOPs: Rioja, Ribera del Duero, Priorat, Rías Baixas, Jumilla, Rueda, Penedès, Navarra, Toro, Bierzo, Somontano — also via EUR-Lex for Jumilla) + **1 EU eAmbrosia/OJ C** (Chianti Classico via EUR-Lex C/2024/1036) + **7 MASAF-subordinate Italian sources** (Barolo & Brunello & Vino Nobile via Valoritalia, Barbaresco & Langhe via Regione Piemonte, Etna via IRVO, Amarone della Valpolicella via Gazzetta Ufficiale). **349 appellation_grapes rows** carry full structured provenance (up from 109 in initial batch). Legal texts extracted from local PDFs via `pypdf`, saved under `data/legal_sources/` (57 files across 3 batches).
