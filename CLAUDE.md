...
- **Path A session (2026-04-05): Seed appellation_rules from legal sources.** Provenance columns added (source_url, source_organization, source_document_title, source_accessed_date, source_text_excerpt, last_verified_at) to `appellation_rules` and `appellation_grapes`. **30 appellation_rules seeded** (0 → 30), all with 100% provenance coverage traceable to legal documents. Source breakdown: 18 INAO cahiers des charges (French AOCs) + 4 MAPA pliegos de condiciones (Rioja DOCa, Ribera del Duero, Priorat/Priorato, Rías Baixas) + 1 EU eAmbrosia/OJ C (Chianti Classico via EUR-Lex C/2024/1036) + **7 MASAF-subordinate Italian sources** (Barolo & Brunello & Vino Nobile via Valoritalia, Barbaresco & Langhe via Regione Piemonte, Etna via IRVO, Amarone della Valpolicella via Gazzetta Ufficiale). **109 appellation_grapes rows** carry full structured provenance. Legal texts extracted from local PDFs via `pypdf`, saved under `data/legal_sources/` (31 files).
  ...
  - **Cascades executed** (strictly definitional, NULL-fills only): **+8,973 wines.color** fills across 15+ single-color regulated appellations (Chablis 560→white, Gevrey-Chambertin 583→red, Pouilly-Fuissé 285, Volnay 272, Vouvray 249, Pommard 366, Pouilly-Fumé 117, Muscadet 28, Pouilly-sur-Loire 6, Chianti Classico 1,283→red, **Barolo 3,218→red, Barbaresco 1,234→red, Brunello 1,273→red, Amarone 535→red, Vino Nobile 197→red**). **+8,599 wines.varietal_category_id** fills (Chablis 2018→Chardonnay, Pouilly-Fuissé 374→Chardonnay, Pouilly-Fumé 106→Sauv Blanc, Sancerre 697→color-dependent, Muscadet 45→Melon de Bourgogne, Vouvray 436→Chenin Blanc, **Barolo 3,233→Nebbiolo, Barbaresco 1,235→Nebbiolo, Brunello 1,274→Sangiovese**). **+7,599 new wine_grapes rows** created at percentage=100 for true 100% single-variety appellations (Chablis 1796, Pouilly-Fuissé 277, Pouilly-Fumé 106, Pouilly-sur-Loire 1, Sancerre 689, **Barolo 3,196, Barbaresco 1,224, Brunello 1,267**). New `source_types.masaf` row added for cascade provenance tracking. Cross-check found **~855 wines with legally impossible colors** (50 Chablis red, 800 Champagne red, 2 Chianti Classico white, 2 Pommard non-red, 1 Gevrey non-red) — left alone per no-overwrite rule, flagged in DECISIONS.md for cleanup session.
