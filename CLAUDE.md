...
- **~432K wines with color (83.5%)** (up from 323K/62.4% via: free fills +15,653 white-grape cascade + +11,171 name-keyword colors, Haiku color classify ~$22 across 4 parallel runs classifying ~30K unique wines at >=0.85 confidence. Scripts: `pipeline/enrich/haiku_color_classify.py`, `pipeline/promote/ttb_color_fill.py`)
...
- **Round 14 follow-ups (2026-04-05):** **+15,940 wine_vintage_formats** (4,697 → 20,637) from Wally's title size parsing. Definitional: format tokens (750mL, 1.5L, 3L, Magnum, etc.) are literally in Wally's title strings, mapped to bottle_formats reference table by volume. **appellation_vintages now populated** (134,877 rows from appellation_weather_years cascade — GDD, rainfall, harvest avg temp, heat spike days, diurnal range, growing season). **appellation_soils: 930 links** across 304 appellations (Haiku extraction from appellation_rules text, $0.34). Other empty tables confirmed: appellation_grapes (0→606 from Path A session → **10,413 total** from Run 2/3 loops covering US AVAs + FR/AU/SA/ES/GR), appellation_rules (0→549 from Path A session → **1,165 total** covering US, Italy, France, Austria, Portugal, Spain, Germany, Greece, Australia, South Africa + 17 other countries from Phase 3 loop — FR/AU/SA/ES/GR all at 0 remaining as of 2026-04-07), wine_vintage_vineyards (0), wine_vintage_documents (0), wine_vintage_descriptors (0), wine_vintage_nv_components (0). All require either external data integration or expert knowledge seeding — not conservative direct-source work.
...
- **DB cleanup + Haiku analysis session (2026-04-06 evening):** Built nightly schema audit (`pipeline/analyze/schema_audit.py`) — 10-category read-only audit, saves JSON to `data/stats/`, appends regressions to `data/stats/schema_audit_journal.md`. Scheduled as nightly task. Conservative fixes: **+8 missing FK indexes** (wines.region_id, wine_vintage_prices.retailer_id, etc.), **+2,278 grape double-space names cleaned** (VIVC import artifacts), **1 vintage year fixed** (2050→2024 parsing artifact). Haiku scripts built and run: **grape_color_haiku.py** — filled 32/268 NULL-color grapes at confidence≥0.85 ($0.009); **wine_dupe_classify.py** — classified all 12,671 duplicate wine groups into match_decisions: 9,600 true_duplicate, 388 distinct_wines, 2,682 unclear ($2.06). Follow-up reclassification of 2,682 unclear: 2,017 reclassified (1,982 true_duplicate, 35 distinct_wines, 665 remain unclear) via `pipeline/enrich/haiku_dupe_reclassify.py` ($0.68). Zero canonical writes — merge backlog now queued in match_decisions.
...
- **Dedup merge session (2026-04-08):** Executed `wine_merge.py` on all 10,469 pending ai_accepted match_decisions groups (25,977 dupe wines). Phase 1 (staging repoints): 0 rows (dupes had no staging links). Phase 2 (canonical child consolidation): 56 vintage conflicts merged, 1,453 child rows re-pointed, 805 external_ids re-pointed, 918 conflict drops, 1,952 NULL fills on survivors. Phase 3 found all dupes already soft-deleted by earlier `dedup_wines.py` run. Backfilled `duplicate_of` on 19,552 wines missing it. 2 errors from `idx_scores_dedup` unique constraint on `wine_vintage_scores` (script handles vintage conflicts but not score-level conflicts — fix needed). **Active wines: 477,151** (40,945 soft-deleted, 34,056 with `duplicate_of`). 156 flagged (unclear) groups remain for future review.
- **Data gap fills + Haiku loop (2026-04-07, ~$23 Haiku):** Free fills: +15,653 white-grape→white color cascade, +11,171 name-keyword color fills (rosé/blanc/tinto/etc.), +134,877 appellation_vintages from weather data (0→134K). Haiku tracks: Track A color classify ~$22 (4 parallel runs, coverage 62.4%→83.5%, +109K wines with color), Track B appellation soils $0.34 (0→930 links across 304 appellations), Track C dupe reclassify $0.68 (2,017/2,682 unclear reclassified: 1,982 true_duplicate + 35 distinct), Track D grape extract $0.35 killed (2% hit rate, not cost-effective). Scripts: `pipeline/enrich/haiku_color_classify.py`, `pipeline/enrich/haiku_appellation_soils.py`, `pipeline/enrich/haiku_dupe_reclassify.py`, `pipeline/enrich/haiku_grape_extract.py`.
...
- **Producer depth** (was all 0): 155 year_established, 185 websites, 117 GPS, 183 descriptions, 110 production, 194 winemaker links
- **Producer website scrape (2026-04-07):** Built `pipeline/fetch/producer_site_scrape.py` — generic Haiku-based extraction from top 100 wine producer websites. 77/100 completed, 23 failed (JS-heavy/bot-protected). **+498 wines, +311 vintages (80 ABV, 36 pH, 36 TA, 135 oak, 252 winemaker notes), +537 wine_grapes, +28 winemakers** (Julie Cavil/Krug, Philippe Guigal, Michel Chapoutier, Véronique Drouhin, Michael Silacci/Opus One, etc.), +55 year_established, +71 descriptions, +113 website_urls on producers. Cost: ~$1.65 Haiku. Top performers: Guigal (23 wines + 29 vintages + 64 grapes), Krug (56 vintages), Ridge (20 wines), Dr. Loosen (18 wines). Failed: most First Growth Bordeaux, Gaja, Vega Sicilia, Screaming Eagle (JS-only sites). Journal: `data/stats/producer_scrape_journal.md`. Known cleanup: 15 vintage-as-name/format-as-name wines (Opus One, Silver Oak) need dedup.
- **Producer scrape v2 improvements (2026-04-07):** Added Playwright fallback (auto-triggers when requests returns <200 chars, handles age gates), Haiku fuzzy wine matching (3-tier: exact -> aggressive normalize -> Haiku, >=80% confidence), `--retry-failed` flag. Retry recovered 7/23 failed producers (Colgin Cellars +25 vintages, DAOU +28 wines, etc.). Fuzzy matching prevented duplicates: "IX Estate" matched "Colgin Cellars, IX Estate Red," at 95%. Total retry: +24 wines, +45 vintages, +23 grapes. Cost: $0.19 Haiku. 16 producers still inaccessible (403 bot protection, SSL errors, heavy SPAs).
- **Accent cleanup (2026-04-07):** Built `pipeline/analyze/accent_cleanup.py` — deterministic accent restoration. **~23,118 names fixed** across producers (4,772: Chateau->Château, Pere->Père, Vina->Viña) and wines (18,346: Cuvee->Cuvée, Cotes->Côtes, Spatlese->Spätlese, Gruner->Grüner, Cremant->Crémant). Zero AI cost. 9,630 slug collisions revealed accent-variant duplicate entries (e.g., "Cuvée Classique" and "Cuvee Classique") — logged as dedup candidates. 28 accent rules covering French, Spanish, Portuguese, German patterns.
- **Knowledge seed (2026-04-08):** Built `pipeline/promote/knowledge_seed.py` — 6-stage pipeline to seed notable wines from Claude training data. 920 generated across 32 categories, 656 already existed (71% overlap), 204 promoted after dual Haiku+Sonnet validation. **+200 wines, +31 producers, +321 wine_grapes.** Backbone IDs: 9/200 (COLA only). Sonnet LWIN matching ($0.22) found 43 same-wine pairs but all conflicted with existing LWIN entries — logged as dedup candidates. Staging table: `source_claude_knowledge`. Total cost: ~$2.50 (Haiku generate ~$1.50, Haiku validate ~$0.50, Sonnet validate ~$0.25, Sonnet LWIN ~$0.22).
