...
### 2026-04-16: Check assumptions with web search — often, early, and before defending a design

Triggered by a multi-turn conversation about LWIN "zero-wine producers" where Claude proposed a bespoke naming fallback chain (`wine_name → sub_region → region`) without first checking whether LWIN's own data dictionary already solved the problem. It did — `source_lwin` ships a `display_name` column populated on 99.994% of rows (including every row Claude was proposing to "recover"), and the column had been there since the initial LWIN import. A 30-second web search of the LWIN guide + a `SELECT column_name FROM information_schema.columns WHERE table_name='source_lwin'` would have surfaced it immediately. Instead: three full back-and-forth turns with the user correcting premises.

**Rule added to CLAUDE.md (Behavioral Instructions):** before making factual claims about external systems (APIs, data formats, standards, third-party schemas), check. Overuse of web search is cheaper than underuse — a superfluous search costs ~5 seconds; a missed one costs a full round-trip and risks anchoring bad design. Also check the local schema via `information_schema.columns` before assuming a column doesn't exist. Trigger is "any non-trivial uncertainty about how an external system represents data," not "only when you're sure you don't know."

### 2026-04-16: LWIN wine recovery scope — fix missing 26,864, backfill display_name on existing 162,458, do NOT rewrite name on existing

Post-discovery that `source_lwin.display_name` is Liv-ex's authoritative combined-name field: decided to (1) create canonical wines for the 26,864 LWIN rows we'd previously skipped (wine_name NULL but display_name populated), using `name = display_name_minus_producer_prefix`, and (2) backfill `wines.display_name` for all 162,458 existing LWIN-linked wines from `source_lwin.display_name` via external_ids join. **Explicitly rejected** rewriting `wines.name` on the existing 162,458 — those were populated correctly from LWIN's `wine_name` field per the LWIN data dictionary, and rewriting would churn slugs (breaking `/wine/{slug}` URLs), trigger full enrichment cache invalidation, risk uniqueness collisions under `(producer_id, name_normalized)`, and deliver no user-visible improvement. wines.display_name is safe to update everywhere (non-unique, not used for slugs, only for rendering). Phrased by user: "we could update LWINs based on LWIN number" — that's exactly the JOIN shape used.

### 2026-04-16: Pre-existing trigger bug on producers+wines search_vector exposed by B6.2

`update_producer_search_vector()` and `update_wine_search_vector()` trigger functions call `build_*_search_vector(NEW.id)` which runs `SELECT ... FROM table WHERE id = NEW.id` — this returns no rows on BEFORE INSERT because the row doesn't physically exist in the heap yet. Result: every INSERTed producer and wine gets `search_vector = NULL` silently. Works fine on UPDATE (row exists). The `update_grape_search_vector()` function is written correctly (uses `NEW.display_name` + `NEW.name` directly). Pre-existing bug not caused by B6.2 — but B6.2 exposed it at scale (+22,598 producers + +42,095 wines with NULL search_vectors). Pre-LWIN rows have 0 NULLs because they were presumably populated by a downstream UPDATE somewhere in the 30K rebuild. Decision: fix in a small pre-B6.3 hygiene pass — patch both trigger functions to compute inline (mirror grape's pattern) + backfill existing NULLs via no-op UPDATE that fires the UPDATE trigger. 7 NULL-country LWIN-meta producers (Sotheby's, LVMH, partnership strings) left for dedup to surface; 5,372 zero-wine LWIN producers (23.8% of new cohort) treated normally by dedup.
...
### 2026-04-17: B6.4 — cross-family agreement is the reliability anchor for dedup

Calibration on 600-pair set (367 gold labels) showed any Haiku-tier + any Gemini-tier both emitting MERGE at conf ≥0.85 hits **100% precision** across 70-78 gold-labeled pairs per cross-family pairing. Same-family (Haiku+Haiku or Gemini+Gemini) had 1 FP each at 98.7%. Individual tier MERGE precisions are already high (L1 Haiku 98.7%, L1.5 Gemini 100%, L2 Haiku rich 97.6%, L2.5 Gemini rich 98.8%) but the cross-family join is what gets us to 100%. This changes the ladder architecture: auto-apply requires cross-family agreement, not single-tier high confidence. Implication: L1.5 Gemini now runs on ALL 151K pairs (not just the 0.92-0.97 L1 band proposed in B6.3). Cost is tiny ($3-4 at 0.023¢/pair).

### 2026-04-17: B6.4 — committed thresholds symmetric + cross-family, MERGE 0.88 SKIP 0.97

Thresholds committed after calibration: Stage 1 MERGE 0.88+0.88 (user chose 0.88 over calibration-supported 0.85 for safety margin) / SKIP 0.97+0.97; Stage 2 MERGE 0.90+0.90 / SKIP 0.95+0.95; L3 MERGE ≥0.92 / SKIP ≥0.90. Symmetric thresholds per B6.3 user preference ("do it right the first time"). The L1 MERGE threshold dropped from B6.3's proposal of 0.97 solo to 0.88 cross-family because Haiku MERGE is consistently ~99% precise across all confidence bands; tightening to 0.97 solo drops recall without gaining precision. What we gain from cross-family agreement is not higher conf bar but orthogonal-error-profile coverage (Haiku cautious, Gemini aggressive — errors don't overlap).

### 2026-04-17: B6.4 — PARENT_CHILD verdicts are noise across all tiers and confidence bands

All 4 calibration tiers had 6-10% PC precision. Raising the confidence threshold does NOT reliably improve PC precision: L1 Haiku PC at 0.85-0.92 is 5.8% (4/69); L1.5 Gemini PC at 0.92-0.97 is 14.8% (4/27). The only PC confidence band with 100% precision was L2.5 Gemini rich PC at ≥0.97 (2/2 — sample too small to trust). What reliably indicates real PC: (a) cross-tier agreement (2+ tiers emit PC), (b) rich-prompt source (L2/L2.5 with full TTB fingerprint + wine catalog see the "distinct brands, shared facility" pattern). Committed PC rule: route to user review iff (a) 2+ tiers emit PC at any conf, OR (b) L2/L2.5 rich PC at conf ≥0.90, OR (c) L3 Sonnet PC at any conf. Single-tier basic-prompt PC (L1 or L1.5 alone) is treated as noise and overridden by cross-tier non-PC consensus. Expected review pile reduction: from 3,000-5,000 to 1,500-3,000 pairs.

### 2026-04-17: B6.4 — L3 web search decision deferred to mid-run based on residual count

L3 ablation on 50 no-web pairs overlapped with oracle (L3 web) labels on only 4 pairs due to `--resume` picking different pairs. Those 4 showed 100% verdict agreement web vs no-web, with 92% cost savings ($0.147 → $0.012/pair). Sample too small to definitively drop web. Decision: make the L3 web choice mid-B6.5a based on actual Stage 2 residual count: <1,000 pairs → use web ($15-30); 1,000-3,000 → use web ($30-90); 3,000-10,000 → L3 no-web bulk + web on sub-sample of disagreements; >10,000 → stop and tighten Stage 2 thresholds before running L3. The rigor tier is where we pay for rigor; don't drop web on thin evidence, but don't blow $1,300+ running web on 10K pairs either.

### 2026-04-17: B6.4 — B6.5 split into B6.5a (automated) + B6.5b (interactive review)

B6.5 in the original plan combined production-scale ladder runs with user review in one session. After calibration revealed a projected 1,500-3,000 pair review pile, the session was split. B6.5a runs L1.5 Gemini basic on 151K → Stage 1 sort → SKIP audit (200 pairs, $5) → L2+L2.5 on escalations → Stage 2 sort → L3 web decision → L3 run → L4 Opus audit → produces `review_queue.json`. Budget $35-80, ~3 hours automated work. B6.5b is a separate 1-3 hour interactive session with the user to review the queue batched by pattern, with Claude-presented context packs + recommendations. Budget $0-5. Reason for split: the two modes of work (automated vs interactive) have different pacing; splitting makes both cleaner and means the user can choose when to do the interactive portion.
...
### 2026-04-21: Producer dedup continues via a broader method bakeoff, not another narrow rescue or plain model rerun

After Session 9.8 recommended freezing at the Session 9.7 layered fallback endpoint, the user explicitly kept Sprint 6 pointed at **production readiness** and authorized a broader redesign as long as it stays within budget. The next continuation should therefore be a **method-class bakeoff** that keeps `benchmark_v1`, the Session 4 hard gates, and the Session 9.7 safety base fixed while comparing materially different adjudication architectures. This is not permission for a plain model zoo, benchmark mutation, or one more benchmark-shaped narrow continuation. The design target is a budget-bounded proof-first bakeoff of broader methods such as router expansion, signature routing, proposer-plus-veto, and evidence-digest variants. Reason: Session 9.8 showed the remaining blocker is architectural, not just a bad last-mile parameter choice, and the sprint goal is still production readiness rather than settling for fallback-only status.

### 2026-04-21: Approve the capped full 152-case rerun of the three Session 9.10 proof survivors

After Session 9.10 showed that `merge_proposer_plus_veto_v1`, `expanded_layered_router_v1`, and `evidence_digest_then_judge_v1` could all survive the 29-case trap-heavy proof subset with `0` false merges, the user approved the next bounded step: run the full frozen `152`-case benchmark using only those three survivors. The approval kept the existing constraints fixed: `benchmark_v1`, the Session 4 production/fallback gates, the Session 9.7 layered safety base, and the `$0` budget posture for a heuristic-only rerun. Reason: the proof subset cleared the minimum bar for one honest full-benchmark check, and the user preferred measuring that directly instead of freezing immediately at the proof stage.
