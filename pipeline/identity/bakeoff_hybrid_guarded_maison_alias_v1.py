"""Session 10.8 - maison/article estate-alias follow-on.

This contender starts from an existing `hybrid_guarded_place_alias_v1` run and
adds one last deterministic probe:

- one side is exactly `de la <token>`
- the other side already exposes the same shared core token
- representative wines on the article-wrapped side explicitly use `Maison de la <token>`
- the packet still lacks exact catalog overlap, so the guarded methods stayed cautious

This is intentionally a single-family probe, not a broad relaxation. The
benchmark currently exposes one positive case of this shape and no negative case
with the same representative-wine wrapper evidence.
"""
...
CONTENDER_ID = "hybrid_guarded_maison_alias_v1"
METHOD_VERSION = "hybrid_guarded_maison_alias_v1"
ARTICLE_WRAPPER_RE = re.compile(r"^de la ([A-Za-z']+)$", re.IGNORECASE)
...
def candidate_applies(packet: dict, row: dict) -> tuple[bool, dict]:
    ...
    name_a = side_a["name"]
    name_b = side_b["name"]
    match_a = ARTICLE_WRAPPER_RE.fullmatch(name_a.strip())
    match_b = ARTICLE_WRAPPER_RE.fullmatch(name_b.strip())

    if bool(match_a) == bool(match_b):
        return False, {}

    article_side = "a" if match_a else "b"
    article_match = match_a or match_b
    assert article_match is not None
    token = article_match.group(1).lower()
    article_catalog_side = side_a if article_side == "a" else side_b
    other_side = side_b if article_side == "a" else side_a
    other_name = other_side["name"].lower()
    rep_wines = article_catalog_side["catalog_summary"].get("representative_wines") or []
    maison_phrase = f"maison de la {token}"
    shared_tokens = [str(value).lower() for value in (lexical.get("shared_core_tokens") or [])]
    wine_count_article = int((side_a if article_side == "a" else side_b)["catalog_summary"].get("wine_count") or 0)
    wine_count_other = int(other_side["catalog_summary"].get("wine_count") or 0)

    applies = (
        geography.get("same_country") is True
        and geography.get("same_region") is False
        and lexical.get("containment") == "none"
        and int(comparison["catalog"].get("exact_overlap_count") or 0) == 0
        and comparison["catalog"].get("portfolio_shape_comment")
        == "The two local catalogs do not expose obvious shared anchors from DB evidence alone."
        and token in shared_tokens
        and token in other_name
        and any(maison_phrase in item.lower() for item in rep_wines)
        and "catalog_asymmetry" in refs
        and "lex_shared_core_tokens" in refs
        and max(wine_count_article, wine_count_other) <= 6
    )
    return applies, {
        "article_side": article_side,
        "estate_token": token,
        "maison_phrase": maison_phrase,
        "wine_count_article": wine_count_article,
        "wine_count_other": wine_count_other,
    }
...
def promote_row(row: dict, packet: dict, details: dict) -> dict:
    ...
    promoted["maison_alias_guard_applied"] = "maison_article_estate_alias_merge"
    promoted["maison_alias_guard_details"] = details
    promoted["normalized_output"] = {
        "packet_id": row["normalized_output"]["packet_id"],
        "verdict": "MERGE",
        "confidence": 0.84,
        "rule_ids": ["11.4.i", "11.6"],
        "reason": (
            "Promoted by `maison_article_estate_alias_merge`: one side is an article-wrapped estate token, "
            f"the other side's representative wines explicitly use `{details['maison_phrase']}`, and the shared core token stays aligned."
        ),
        "key_support_refs": ["catalog_asymmetry", "geo_same_country", "lex_shared_core_tokens"],
...
def render_markdown(report: dict) -> str:
    ...
    if report["changed_cases"]:
        for item in report["changed_cases"]:
            lines.append(
                f"- `{item['case_id']}`: `{item['source_verdict']}` -> `{item['new_verdict']}` via `{item['rule']}` ({item['maison_phrase']})"
            )
    else:
        lines.append("- none")
    lines.extend(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply maison/article estate-alias promotion on top of a guarded place-alias run.")
    parser.add_argument("--source-report", required=True, help="Path to a session10_8_hybrid_guarded_place_alias_*.json report.")
            ...
            changed_cases.append(
                {
                    ...
                    "rule": "maison_article_estate_alias_merge",
                    "maison_phrase": details["maison_phrase"],
                }
            )
        else:
    ...
    report = {
        ...
        "recommendation": {
            "status": (
                "candidate_improves_place_alias_margin"
                if score["gates"]["production"]["status"] == "pass"
                and score["counts"]["false_merge"] == 0
                and score["counts"]["hard_missed_merge"] < source_report["score"]["counts"]["hard_missed_merge"]
                else "candidate_not_better_than_place_alias"
            ),
            "reason": (
                "The maison/article estate-alias probe improved the place-alias leader without reopening false merges."
                if score["gates"]["production"]["status"] == "pass"
                and score["counts"]["false_merge"] == 0
                and score["counts"]["hard_missed_merge"] < source_report["score"]["counts"]["hard_missed_merge"]
                else "The maison/article probe did not produce a cleaner benchmark result than the place-alias source."
            ),
        },
    }
