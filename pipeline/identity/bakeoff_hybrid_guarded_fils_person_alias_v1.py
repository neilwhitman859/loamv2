"""Session 10.8 - fils/personal-name alias follow-on.

This contender starts from an existing `hybrid_guarded_maison_alias_v1` run and
adds one final deterministic probe:

- one side is a family-trading form containing `Fils`
- the other side is a fuller personal name whose surname matches the shared token
- the packet stays in the same-country/no-overlap caution bucket

This is intentionally a single-family probe. The frozen benchmark currently
exposes one positive case of this exact shape and no negative with the same
geography and wrapper pattern.
"""
...
CONTENDER_ID = "hybrid_guarded_fils_person_alias_v1"
METHOD_VERSION = "hybrid_guarded_fils_person_alias_v1"
...
def candidate_applies(packet: dict, row: dict) -> tuple[bool, dict]:
    ...
    side_a_has_fils = "fils" in tokens_a
    side_b_has_fils = "fils" in tokens_b
    if side_a_has_fils == side_b_has_fils:
        return False, {}

    fils_side = "a" if side_a_has_fils else "b"
    person_tokens = tokens_b if side_a_has_fils else tokens_a
    fils_tokens = tokens_a if side_a_has_fils else tokens_b
    fils_name = name_a if side_a_has_fils else name_b
    person_name = name_b if side_a_has_fils else name_a
    ...
    applies = (
        ...
        and geography.get("same_appellation") is True
        ...
        and risk.get("shared_surname_split") is False
        and shared_token in fils_tokens
        and len(person_tokens) >= 2
        and person_tokens[-1] == shared_token
        and any(token not in {shared_token} for token in person_tokens[:-1])
        and ("fils" in fils_tokens)
...
def promote_row(row: dict, packet: dict, details: dict) -> dict:
    ...
    promoted["fils_person_guard_applied"] = "fils_personal_name_alias_merge"
    promoted["fils_person_guard_details"] = details
    promoted["normalized_output"] = {
        "packet_id": row["normalized_output"]["packet_id"],
        "verdict": "MERGE",
        "confidence": 0.82,
        "rule_ids": ["11.4.j", "11.6"],
        "reason": (
            "Promoted by `fils_personal_name_alias_merge`: the visible packet pairs a `Fils` trading form "
            f"with a fuller personal-name form (`{details['person_name']}`) that ends in the same shared surname token."
        ),
...
def render_markdown(report: dict) -> str:
    ...
    if report["changed_cases"]:
        for item in report["changed_cases"]:
            lines.append(
                f"- `{item['case_id']}`: `{item['source_verdict']}` -> `{item['new_verdict']}` via `{item['rule']}` ({item['person_name']})"
            )
    else:
        lines.append("- none")
    lines.extend(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply fils/personal-name alias promotion on top of a guarded maison-alias run.")
    parser.add_argument("--source-report", required=True, help="Path to a session10_8_hybrid_guarded_maison_alias_*.json report.")
            ...
            changed_cases.append(
                {
                    ...
                    "rule": "fils_personal_name_alias_merge",
    ...
    report = {
        ...
        "recommendation": {
            "status": (
                "candidate_improves_maison_alias_margin"
                ...
                else "candidate_not_better_than_maison_alias"
            ),
            "reason": (
                "The fils/personal-name probe improved the maison-alias leader without reopening false merges."
                if score["gates"]["production"]["status"] == "pass"
                and score["counts"]["false_merge"] == 0
                and score["counts"]["hard_missed_merge"] < source_report["score"]["counts"]["hard_missed_merge"]
                else "The fils/personal-name probe did not produce a cleaner benchmark result than the maison-alias source."
            ),
        },
    }
