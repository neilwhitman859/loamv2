...
Usage:
    python -m pipeline.load.tabc_staging
"""
...
def main():
    ...
    for i in range(0, len(unique), BATCH_SIZE):
        ...
        try:
            result = sb.table("source_tabc").upsert(
                batch, on_conflict="ttb_number"
            ).execute()
    ...
    result = sb.table("source_tabc").select("*", count="exact", head=True).execute()
