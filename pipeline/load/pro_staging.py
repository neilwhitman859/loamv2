...
Usage:
    python -m pipeline.load.pro_staging                    # load all parsed states
    python -m pipeline.load.pro_staging --state ar,co      # load specific states
    python -m pipeline.load.pro_staging --dry-run          # count only, no insert
"""
...
def main():
    ...
    parser.add_argument("--state", type=str, help="Comma-separated state codes")
    ...
    if dry_run:
        ...
        for n in sorted(state_counts.keys()):
    ...
    result = sb.table("source_pro_platform").select("*", count="exact", head=True).execute()
