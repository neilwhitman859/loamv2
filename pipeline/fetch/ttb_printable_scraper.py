#!/usr/bin/env python3
"""
TTB COLA Printable Page Scraper

Scrapes printable version pages for fields not on the detail page:
  - Wine appellation
  - Grape varietals (higher quality than detail page)
  - Applicant name/address/city/state/zip
  - Phone, email, qualifications

Uses the same Chrome inject approach as ttb_chrome_scraper.py
but hits publicFormDisplay URLs and runs on port 8766.

Usage:
    python -m pipeline.fetch.ttb_printable_scraper [options]

    Then inject: pipeline/fetch/ttb_printable_inject.js
"""
...
DEFAULT_OUTPUT = Path.home() / "Desktop" / "Loam Cowork" / "data" / "imports" / "ttb_cola_printable"
DB_BATCH_SIZE = 100
SERVER_PORT = 8766
            ...
            if self.path == "/results":
                ...
                try:
                    ...
                    for r in results:
                        ...
                        row = {
                            "ttb_id": ttb_id,
                            "printable_scraped_at": "now()",
                        }

                        fields = r.get("fields", {})

                        # All form fields — both old and new form versions
                        text_fields = [
                            # Common fields
                            "rep_id_no", "permit_number", "source_of_product",
                            "serial_number", "class_type_desc", "brand_name",
                            "fanciful_name", "mailing_address", "formula",
                            "grape_varietals", "wine_appellation", "wine_vintage",
                            "phone", "email", "type_of_application",
                            "date_of_application", "applicant_name", "approval_date",
                            "qualifications", "expiration_date",
                            "applicant_address", "applicant_city",
                            "applicant_state", "applicant_zip",
                            "total_bottle_capacity",
                            "ct_code", "or_code", "label_dimensions",
                            "label_details", "field_15",
                            # Field 8 - full applicant block with DBA
                            "applicant_dba",
                            # Old form fields
                            "abv", "net_contents", "fax_number", "lab_no",
                        ]
...
def main():
    ...
    # ── Checkpoint ──
    checkpoint_file = output_dir / "checkpoint_printable.txt"
    ...
    console.print(f"[yellow]  pipeline/fetch/ttb_printable_inject.js[/yellow]\n")
