...
interface CountryInsight {
  ...
  ai_signature_styles: string | null
  ai_regulatory_overview: string | null
}
...
export default function CountryPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('country_insights')
        .select('ai_overview, ai_wine_history, ai_key_regions, ai_signature_styles, ai_regulatory_overview')
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 pb-16">
      ...
      {insight?.ai_wine_history && (
        <Section title="Wine history">
          <p className="text-sm text-earth-600">{insight.ai_wine_history} <AiLabel /></p>
        </Section>
      )}

      {insight?.ai_key_regions && (
        <Section title="Key regions">
          <p className="text-sm text-earth-600">{insight.ai_key_regions} <AiLabel /></p>
        </Section>
      )}

      {insight?.ai_signature_styles && (
        <Section title="Signature styles">
          <p className="text-sm text-earth-600">{insight.ai_signature_styles} <AiLabel /></p>
        </Section>
      )}
    </div>
  )
