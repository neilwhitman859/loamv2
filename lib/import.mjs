...
class ReferenceData {
  ...
  async load() {
    ...
    for (const cl of clLevels) {
      ...
      this.classificationLevels.set(key, { levelId: cl.id, systemName: sys.name, levelName: cl.level_name, rank: cl.level_rank });
  ...
  resolveGrape(name) {
    ...
    // 1. Check alias table
    const aliased = GRAPE_ALIASES[lower];
    if (aliased) {
      const g = this.grapes.get(aliased.toLowerCase());
      if (g) return g;
    }
    // 2. Check display_name
    const byDisplay = this.grapes.get(lower);
    if (byDisplay) return byDisplay;
    // 3. Check VIVC name
    const byVivc = this.grapesByName.get(lower);
    if (byVivc) return byVivc;
    // 4. Check synonyms table
    const synId = this.grapeSynonyms.get(lower);
    if (synId) {
    ...
    for (const suffix of [' noir', ' blanc', ' tinto', ' tinta', ' blanco']) {
      const withSuffix = this.grapes.get(lower + suffix);
  ...
  resolveRegion(name, countryId) {
    if (!name) return null;
    const lower = name.toLowerCase().trim();
    // Try with country disambiguation first
    if (countryId) {
      const r = this.regions.get(`${lower}|${countryId}`);
      if (r) return r;
    }
    const r = this.regions.get(lower);
    return r || null;
  }
...
class ProducerImporter {
  ...
  async insertFarmingCert(cert) {
    // Look up certification by name
    const { data: certRow } = await sb.from('farming_certifications')
      .select('id').ilike('name', cert.name).single();
    if (!certRow) {
      this.warn(`Farming certification not found: "${cert.name}"`);
      return;
    }
    if (this.dryRun) {
      console.log(`[DRY RUN] Would add certification: ${cert.name}`);
      return;
    }
    const { error } = await sb.from('producer_farming_certifications').insert({
      producer_id: this.producerId,
      farming_certification_id: certRow.id,
      certified_since: cert.since || null,
  ...
  async importWine(wine) {
    ...
    if (existing) {
      ...
      if (!varietalCategoryId) {
        const fallback = (wine.color === 'white') ? 'white-blend' : 'red-blend';
        varietalCategoryId = this.refs.resolveVarietalCategory(fallback);
        this.warn(`No varietal category for ${wine.name}, defaulting to ${fallback}`);
      }
      ...
      const row = {
        ...
        wine_type: wine.wine_type || 'table',
        sweetness_level: wine.sweetness_level || null,
        effervescence: wine.effervescence || 'still',
        vineyard_name: wine.vineyard_name || null,
        oak_origin: wine.oak_origin || null,
        yeast_type: wine.yeast_type || null,
        fining: wine.fining || null,
        filtration: wine.filtration ?? null,
        closure: wine.closure || null,
        fermentation_vessel: wine.fermentation_vessel || null,
  ...
  async importWineGrapes(wineId, wineName, grapes) {
    for (const g of grapes) {
      const grape = this.refs.resolveGrape(g.name);
      if (!grape) {
        this.warn(`Unknown grape: "${g.name}" for ${wineName}`);
        continue;
      }
      ...
      if (error && !error.message.includes('duplicate')) {
        this.warn(`Wine grape error for ${wineName}/${g.name}: ${error.message}`);
      } else if (!error) {
  ...
  async importVintage(wineId, wine, v) {
    ...
    const row = {
      wine_id: wineId,
      vintage_year: v.year || null,
      abv: v.abv || null,
      alcohol_pct: v.abv || null,
      ph: v.ph || null,
      ta_g_l: v.ta_g_l || null,
      rs_g_l: v.rs_g_l || null,
    ...
    if (v.grapes && v.grapes.length > 0) {
      for (const g of v.grapes) {
        const grape = this.refs.resolveGrape(g.name);
        if (!grape) {
          this.warn(`Unknown vintage grape: "${g.name}" for ${wine.name} ${v.year}`);
          continue;
        }
        if (this.dryRun) continue;
