...
if (!jsonPath) {
  console.error('Usage: node lib/import.mjs <path-to-json> [--dry-run] [--replace] [--validate]');
  process.exit(1);
}
...
function cleanMetadata(meta, keysToStrip) {
...
const GRAPE_ALIASES = {
  ...
  'chenin': 'Chenin Blanc',
  'steen': 'Chenin Blanc',  // South African name
  'viognier': 'Viognier',
  'verdejo': 'Verdejo',
  'albariño': 'Albariño',
  'albarino': 'Albariño',
  ...
  'cinsaut': 'Cinsaut',
  'cinsault': 'Cinsaut',
  'hermitage': 'Cinsaut',  // old SA name
  // Georgian
  'rkatsiteli': 'Rkatsiteli',
  'saperavi': 'Saperavi',
  'mtsvane': 'Mtsvane Kakhuri',
  'mtsvane kakhuri': 'Mtsvane Kakhuri',
  'kisi': 'Kisi',
  // Lebanese
  'obaideh': 'Obaideh',
  'merwah': 'Merwah',
...
const REGION_ALIASES = {
  ...
  'macedonia': 'Macedonia',
  // Lebanese regions (Bekaa Valley is the main wine region — if not in DB, will need adding)
  ...
  'swartland': 'Swartland',
  'hemel-en-aarde': 'Hemel-en-Aarde Valley',
  'elgin': 'Elgin',
  ...
  'santa cruz mountains': 'Santa Cruz Mountains',
  // Chilean regions
...
class ReferenceData {
  ...
  async load() {
    ...
    for (const a of appellations) {
      ...
      // Also store normalized (accent-stripped, hyphen→space) version
      const norm = normalize(a.name);
      if (!this.appellations.has(norm)) {
        this.appellations.set(norm, a);
      }
    }
    ...
    for (const ld of lds) {
      const cid = ld.country_id || 'null';
      // Key by canonical_name|country_id for disambiguation
      this.labelDesignations.set(`${ld.canonical_name.toLowerCase()}|${cid}`, ld.id);
      // Also key by normalized canonical name
      const normCanonical = normalize(ld.canonical_name);
      if (normCanonical !== ld.canonical_name.toLowerCase()) {
        this.labelDesignations.set(`${normCanonical}|${cid}`, ld.id);
      }
      if (ld.local_name) {
        this.labelDesignations.set(`${ld.local_name.toLowerCase()}|${cid}`, ld.id);
        const normLocal = normalize(ld.local_name);
        if (normLocal !== ld.local_name.toLowerCase()) {
          this.labelDesignations.set(`${normLocal}|${cid}`, ld.id);
        }
      }
    }
  ...
  resolveLabelDesignation(name, countryId) {
    ...
    const lower = name.toLowerCase().trim();
    const norm = normalize(lower);
    // Try country-specific first (both exact and normalized)
    if (countryId) {
      const id = this.labelDesignations.get(`${lower}|${countryId}`) ||
                 this.labelDesignations.get(`${norm}|${countryId}`);
      if (id) return id;
    }
    // Try universal (null country)
    const universal = this.labelDesignations.get(`${lower}|null`) ||
                      this.labelDesignations.get(`${norm}|null`);
    if (universal) return universal;
    // Fallback: try any country (designation used cross-border)
    for (const [key, id] of this.labelDesignations.entries()) {
      if (key.startsWith(`${lower}|`) || key.startsWith(`${norm}|`)) return id;
    }
    return null;
  }
...
class ProducerImporter {
  ...
  validate() {
    ...
    // Producer checks
    if (!p.name) issues.push('CRITICAL: producer.name is required');
    if (!p.country) issues.push('CRITICAL: producer.country is required');
    const validTypes = ['estate', 'negociant', 'cooperative', 'virtual', 'corporate'];
    if (p.producer_type && !validTypes.includes(p.producer_type)) {
      issues.push(`ERROR: producer_type "${p.producer_type}" not in ${validTypes.join('/')}`);
    }

    // Wine checks
    const validColors = ['red', 'white', 'rose', 'orange'];
    const validWineTypes = ['table', 'sparkling', 'dessert', 'fortified', 'aromatized'];
    const validEffervescence = ['still', 'sparkling', 'semi_sparkling'];

    for (const wine of (this.data.wines || [])) {
      const wn = wine.name || '(unnamed)';
      if (!wine.name) issues.push(`CRITICAL: wine missing name`);
      if (wine.color && !validColors.includes(wine.color)) {
        issues.push(`ERROR: ${wn} — color "${wine.color}" not in ${validColors.join('/')}${wine.color === 'rosé' ? ' (use "rose" without accent)' : ''}`);
      }
      if (wine.wine_type === 'still') {
        issues.push(`WARN: ${wn} — wine_type "still" will be auto-corrected to "table"`);
      } else if (wine.wine_type && !validWineTypes.includes(wine.wine_type)) {
        issues.push(`ERROR: ${wn} — wine_type "${wine.wine_type}" not in ${validWineTypes.join('/')}`);
      }
      if (wine.effervescence && !validEffervescence.includes(wine.effervescence)) {
        issues.push(`ERROR: ${wn} — effervescence "${wine.effervescence}" not in ${validEffervescence.join('/')}`);
      }

      // Grape checks
      for (const g of (wine.grapes || [])) {
        const gn = g.name || g.grape;
        if (!gn) issues.push(`ERROR: ${wn} — grape entry missing name/grape field`);
      }

      // Vintage checks
      for (const v of (wine.vintages || [])) {
        if (v.year == null) issues.push(`WARN: ${wn} — vintage missing year (use 0 for NV)`);
        if (v.abv && (v.abv < 0 || v.abv > 25)) {
          issues.push(`WARN: ${wn} ${v.year} — unusual ABV: ${v.abv}%`);
        }
        if (v.rs_g_l && v.rs_g_l > 500) {
          issues.push(`WARN: ${wn} ${v.year} — very high RS: ${v.rs_g_l} g/L`);
        }
        // Score checks
        for (const s of (v.scores || [])) {
          if (!s.publication) issues.push(`ERROR: ${wn} ${v.year} — score missing publication`);
          if (s.score) {
            const scale = s.scale || '100';
            const maxScore = scale === '20' ? 20 : scale === '5' ? 5 : 100;
            const minScore = scale === '20' ? 10 : scale === '5' ? 1 : 50;
            if (s.score < minScore || s.score > maxScore) {
              issues.push(`WARN: ${wn} ${v.year} — unusual score: ${s.score} (scale: ${scale})`);
            }
          }
        }
      }
    ...
    if (issues.length > 0) {
      ...
      for (const i of issues) {
        const icon = i.startsWith('CRITICAL') ? '🔴' : i.startsWith('ERROR') ? '🟠' : '🟡';
        console.log(`  ${icon} ${i}`);
      }
      const criticals = issues.filter(i => i.startsWith('CRITICAL'));
      if (criticals.length > 0) {
        throw new Error(`${criticals.length} critical validation error(s) — fix before importing`);
      }
      console.log('');
    }
    return issues;
  ...
  async importProducer() {
    ...
    const row = {
      ...
      metadata: cleanMetadata(p.metadata, [
        'location', 'address', 'philosophy', 'total_production_cases',
        'annual_production', 'founded_year', 'year_established',
      ]),
    };
  ...
  async importWine(wine) {
    ...
    if (existing) {
      ...
      const row = {
        ...
        soil_description: wine.soil_description || wine.soil || null,
        vine_age_description: wine.vine_age_description || wine.vine_age || null,
        vineyard_area_ha: wine.vineyard_area_ha || wine.vineyard_area || null,
        commune: wine.commune || null,
        altitude_m_low: wine.altitude_m_low || (wine.altitude ? parseAltitude(wine.altitude).low : null),
        altitude_m_high: wine.altitude_m_high || (wine.altitude ? parseAltitude(wine.altitude).high : null),
        aspect: wine.aspect || null,
        slope_pct: wine.slope_pct || null,
        monopole: wine.monopole || false,
        metadata: cleanMetadata(wine.metadata, [
          'soil', 'vine_age', 'vineyard_area', 'commune', 'altitude',
          'aspect', 'slope', 'monopole', 'vinification', 'style',
          'first_vintage', 'first_vintage_year', 'description',
        ]),
      };
  ...
  async importVintage(wineId, wine, v) {
    ...
    if (existing) {
      ...
      if (v.scores) {
        for (const s of v.scores) {
          await this.importScore(wineId, v.year, s, existing.id);
        }
      }
      return;
    }
    ...
    if (v.scores) {
      for (const s of v.scores) {
        await this.importScore(wineId, v.year, s, vintageId);
      }
    }
  ...
  async importScore(wineId, vintageYear, s, vintageId = null) {
    ...
    const { error } = await sb.from('wine_vintage_scores').insert({
      ...
      critic_drinking_window_start: s.drinking_window_start || s.drink_from || null,
      critic_drinking_window_end: s.drinking_window_end || s.drink_to || null,
      source_id: this.sourceId,
...
async function main() {
  ...
  if (VALIDATE_ONLY) {
    const issues = importer.validate();
    console.log(`\nValidation complete: ${issues.length} issue(s) found.`);
    process.exit(issues.filter(i => i.startsWith('CRITICAL') || i.startsWith('ERROR')).length > 0 ? 1 : 0);
  }
