...
**Sprint sequence:** 1 (Build, done) → 2 (Audit, done) → 3 (Fix, done) → 4 (Demo, done) → 5 (AI Bakeoff, done) → **6 (active): LWIN import + producer dedup — current path is the merge-only Codex rebuild. The frozen Session 4 adjudication bakeoff has now been rerun in v2 and then audited in Session 9; queue-building remains blocked pending a v3 continuity-tightening proof rerun** → Sprint 7: wine dedup → Sprint 8: prompt v2 + L3 fact-check gate + re-enrichment + share. The bake-off ranked models under the *current* prompt; re-enrichment is deferred until prompt + gate work lands so we don't bake in the current-prompt ceiling.
...
- inspect the residual Session 9.1 proof failures (`blind_core_audit_048`, `blind_core_audit_080`, `tail_random_sample_008`) and design the minimum unresolved-official / secondary-evidence backstop needed before another proof-only rerun; no full 152-case rerun or queue-building until that proof passes
...
**Session 9.1 proof outcome (2026-04-21):** the minimum v3 continuity redesign is now implemented in `pipeline/identity/bakeoff_packet_v2.py`, `pipeline/identity/bakeoff_harness_v2.py`, and `pipeline/identity/bakeoff_run_v2.py`, with a proof-only runner path that requires explicit approval before any future full rerun. Fresh proof artifacts live under `data/sprints/dedup/bakeoff_v2/` with packet dir `packets_session9_v3_continuity/` and run name `session9_v3_continuity_proof_subset`. Result: **proof failed cleanly, and the 152-case rerun did not execute.** The reused 28-case base improved sharply (`sonnet_guardrailed_v2` false merges 10→1, `gemini_guardrailed_v2` 7→0, `sonnet_gemini_consensus_v2` 7→0), but the add-on continuity slice still posted residual false merges: case `blind_core_audit_048` failed for Sonnet, Gemini, and consensus; case `blind_core_audit_080` failed for Sonnet only; case `tail_random_sample_008` failed for Gemini only. The remaining pattern is no longer bogus hard continuity. It is **secondary/catalog overreach under unresolved-official evidence**: these proof failures merged on secondary hits plus local catalog coherence even though the packet had no hard continuity and only `risk_sparse_official_evidence` as a backstop. Queue-building and the full 152-case rerun remain blocked.
