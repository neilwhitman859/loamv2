...
**Sprint sequence:** 1 (Build, done) → 2 (Audit, done) → 3 (Fix, done) → 4 (Demo, done) → 5 (AI Bakeoff, done) → **6 (closed): LWIN import + pairwise producer dedup rebuild. Sprint 6 produced the benchmark, packet, and failure-mode corpus, but it never cleared the trust bar for execution. The best surviving artifact remains `session9_7_layered_safety_sonnet_r2_narrow`, preserved as the safest non-production benchmark control.** → **7 (active): Producer identity ER around `identity_dossier_select_v1` ("Identity Dossier Select"), a dossier-based shortlist-and-select method with AI-only escalation and no human pair review at scale.** → Sprint 8: wine dedup → Sprint 9: prompt v2 + L3 fact-check gate + re-enrichment + share. The bake-off ranked models under the *current* prompt; re-enrichment is deferred until prompt + gate work lands so we don't bake in the current-prompt ceiling.
**Session 9.7 addendum (2026-04-21):** the user explicitly opened the broader redesign after the failed Session 9.6 specialist proof. The new best artifact is `session9_7_layered_safety_sonnet_r2_narrow`, which clears the fallback gate with `0` false merges overall but still fails the frozen production gate.
**Session 9.8 addendum (2026-04-21):** Session 9.8 audited the remaining miss set in that layered fallback and found no honest narrow recall-only continuation. The remaining blocker still needs at least `5` more safe recoveries, spans too many family/signature shapes to count as one more narrow proof, and includes `4` misses outside the current routed-family specialist bundle. Queue-building remains blocked; the recommended posture is now freeze at the Session 9.7 layered fallback endpoint unless the user explicitly opens a broader multi-family redesign.
**Session 9.9 addendum (2026-04-21):** the user explicitly rejected fallback-only closure as the sprint endpoint and kept Sprint 6 aimed at production readiness. The next approved continuation is a broader **method bakeoff** that keeps `benchmark_v1`, the Session 4 gates, and the Session 9.7 layered safety base fixed while comparing a small number of materially different adjudication architectures. The next execution step is a bounded proof subset, not queue-building.
**Session 9.10 addendum (2026-04-21):** Session 9.10 ran that bounded proof subset on `29` trap-heavy cases at `$0` new model spend. Result: `merge_proposer_plus_veto_v1`, `expanded_layered_router_v1`, and `evidence_digest_then_judge_v1` all survived cleanly with `0` false merges and `3-4 / 5` blind-core blocker recoveries; `signature_router_v1` matched `expanded_layered_router_v1` exactly and was dropped as redundant. Queue-building is still blocked, but the next honest move is now a capped full `152`-case rerun of only those three survivors rather than a freeze at the Session 9.7 fallback endpoint.
**Session 9.11 addendum (2026-04-21):** Session 9.11 ran that capped full `152`-case rerun at `$0` new model spend. Result: all three broader methods recovered `3-5` missed merges relative to the Session 9.7 control, but they also reopened `5-9` false merges at full scale, so none cleared the frozen production or fallback gates. The best surviving artifact remains `session9_7_layered_safety_sonnet_r2_narrow`; queue-building stays blocked, and the recommended posture is now freeze at that best non-production artifact rather than opening another redesign in the same session.
**Session 9.12 planning addendum (2026-04-21):** after reviewing the Session 9.11 result, the user redirected the next session away from an automatic freeze-closeout memo and toward a higher-level viability review of Loam itself. The key question is no longer only which artifact survives; it is whether producer dedup is fundamental enough that failing it should freeze or shut down the project rather than trigger more redesign by default. Until that discussion lands, queue-building remains blocked and `session9_7_layered_safety_sonnet_r2_narrow` is only an interim best non-production artifact, not an implied go-forward plan.
**Session 9.12 result addendum (2026-04-21):** the viability review concluded that the current producer-dedup continuation is spent. The recommended posture is now freeze-first rather than shutdown-by-default or redesign-by-default: preserve `session9_7_layered_safety_sonnet_r2_narrow` as the best surviving non-production artifact, forbid new reruns or queue-building on the current line, and make the next session an explicit founder-level decision about whether Loam continues under a narrower honest scope or gets shelved.
**Sprint 7 launch addendum (2026-04-21):** the user chose to continue Loam through a new producer-identity sprint instead of a freeze/shelve decision or any human-review queue. New hard constraints: no human pair review at scale, fixed budget, AI-only escalation only on a narrow frontier, and abstention preferred over risky merges. The new live method family is `identity_dossier_select_v1`: build reusable producer dossiers, generate cheap candidate shortlists, ask the model to select best match or `none`, emit explicit `SAME_AS` / `RELATED_BUT_DISTINCT` / `NONE` / `UNSURE` outputs, and allow merges only from accepted `SAME_AS` edges. Sprint 6 remains preserved as historical benchmark context; Sprint 7 is now the active path.
**Session 10.2 addendum (2026-04-21):** Session 10.2 turned Sprint 7's method name into a durable evidence spec. New artifacts: `data/sprints/identity-er/producer_dossier_v1.md`, `source_signal_inventory.md`, and `edge_taxonomy_v1.md`. The cheap dossier is explicitly backbone-first and regulatory-aware because the canonical producer table is still sparse (`producer_aliases = 0`, `producer_importers = 0`, producer-level `external_ids = 0`, canonical `website_url = 14`), so v1 leans on names, place, portfolio shape, and US regulatory/market clues instead of pretending website/history data exists at scale. `RELATED_BUT_DISTINCT` is now locked as a first-class stored non-merge edge, and only accepted `SAME_AS` edges are merge-eligible. The next honest step is shortlist generation design against that fixed dossier object.
**Session 10.3 addendum (2026-04-21):** Session 10.3 fixed the candidate-retrieval contract in `data/sprints/identity-er/shortlist_generation_v1.md`. `v1` shortlist generation is now explicitly Tier-A-only for seeding: canonical lexical blocking, LWIN producer names, TTB brand/applicant names, and state-registration brand/legal names may create candidate IDs; importer, retailer, merchant, website, profile, and history sources may only re-rank or feed escalation after a candidate already exists. The raw candidate union is capped at `24`, the final selector shortlist is capped at `12`, and the suppression layer must drop weak fuzzy hits, shared-surname / first-word collisions, permit-only adjacency, and merchant/importer-context-only candidates before selector work. The Session 10.2a autonomy blocker is now cleared; the next honest step is selector-harness design against the fixed dossier + shortlist contracts.
**Session 10.4 addendum (2026-04-21):** Session 10.4 fixed the selector layer in `data/sprints/identity-er/selector_harness_v1.md`. The selector now sees one anchor selector card plus up to `12` candidate mini-cards with explicit comparison blocks, must choose one candidate or `none` before applying `SAME_AS` / `RELATED_BUT_DISTINCT` / `NONE` / `UNSURE`, and uses patterned reason codes plus packet-visible evidence references instead of prose. `NONE` is now explicitly the stop condition, while `UNSURE` is reserved for cases where escalation may still change the answer. The bounded proof is also now split into a selector-only frozen-packet phase and a shortlist-integration smoke phase so retrieval failures cannot hide inside selector scoring. The next honest step is Session 10.5: narrow escalation-layer and accepted-edge-rule design against that fixed selector contract.
**Session 10.5 addendum (2026-04-21):** Session 10.5 froze the Sprint 7 control package in `data/sprints/identity-er/escalation_dossier_v1.md`, `accepted_edge_rules_v1.md`, and `selector_proof_v1.md`. Escalation is now `UNSURE`-only, one-pass, and limited to `candidate_frontier` or `shortlist_gap_probe`; it may add only bounded website/profile/history/raw-row blocks and it keeps the same four-label contract as the cheap selector. Durable graph state is now split between accepted `SAME_AS` / `RELATED_BUT_DISTINCT` / `NONE` edges and non-edge frontier cases; selector-side `NONE` fans out only to candidates actually shown in the packet, while empty shortlists and post-escalation `UNSURE` stay out of the accepted-edge graph. The next honest step is Session 10.6: build the local proof bundle, scorer, and accepted-edge write simulator for the frozen 48-case selector proof plus the 8-case escalation subset, with no policy changes or DB writes.
**Session 10.6 addendum (2026-04-21):** Session 10.6 built the frozen local `selector_proof_v1` bundle under `data/sprints/identity-er/proof/` and the first execution scaffolding in `pipeline/identity/selector_proof_v1.py`. The bundle now persists the case-source manifest, hidden answer key, `48` Phase A selector packets, `8` Phase B escalation packets, Phase C shortlist smoke manifest, normalized result schema, scorecard template, build memo, and accepted-edge/frontier write simulation, all generated from live read-only DB data with no model calls and no DB writes. Oracle self-check passes all four local phases, which means the scaffolding is coherent but does **not** yet prove actual selector or escalation quality. The next honest step is Session 10.7: run the first bounded proof execution against the frozen packets, score the real outputs, determine whether Phase C is runnable from existing shortlist code, and write the Sprint 7 go / no-go memo without reopening policy.
**Session 10.7 addendum (2026-04-21):** Session 10.7 ran the first real bounded proof on the frozen `selector_proof_v1` bundle using `claude-sonnet-4-6` at about `$2.08`, writing normalized Phase A / Phase B results, raw response logs, a scored execution summary, a filled scorecard, an explicit Phase C runnable/block note, and the Sprint 7 go / no-go memo under `data/sprints/identity-er/proof/`. Result: **`NO_GO`**. Phase A stayed safe on false `SAME_AS` (`0`) but still missed `7 / 16` `SAME_AS`, missed all `12 / 12` `RELATED_BUT_DISTINCT`, and resolved all `8` frontier cheap-path `UNSURE` cases to non-`UNSURE` labels. Phase B recovered only `4 / 6` resolvable frontier cases, reopened `1` false `RELATED_BUT_DISTINCT`, and still resolved the `2` expected-`UNSURE` shortlist-gap probes to `NONE`. Phase D reopened `4` contradictory overwrite attempts because escalation tried to recover cases the selector had already written as `NONE`, and Phase C remains blocked because no honest reusable `shortlist_generation_v1` runner exists outside the proof scaffolding. The next honest step is not builder implementation; it is an explicit user decision on whether to run a narrow failure-analysis session or freeze Sprint 7 here.
**Session 10.8 planning addendum (2026-04-21):** the user explicitly rejected the narrow failure-analysis-only continuation and reopened Sprint 7 as an **11-hour autonomous broad method bakeoff**. Keep `benchmark_v1`, the frozen Session 4 production/fallback gates, `no DB writes`, `no human pair review at scale`, and the rule that abstention beats a false merge fixed. Treat `identity_dossier_select_v1` as one failed contender, not the live frame. The new loop is: strategize materially different method families, pick the most promising candidate, build the bounded proof needed to test it, and if any method appears to clear the frozen production gate, run stricter confirmation before trusting it. Operate with 15-minute thread heartbeats and a hard `$30` incremental budget ceiling for the autonomous run.
**Session 10.8 result addendum (2026-04-22):** Session 10.8 completed that broad bakeoff and produced a durable ranked ledger under `data/sprints/identity-er/method_bakeoff/`. The strongest frozen-benchmark survivor became `hybrid_guarded_fils_person_alias_v1`: a guarded frontier base plus three zero-cost overlays that cleared the full frozen benchmark on Sonnet, Sonnet rerun, Opus, and Opus rerun at `0` false merges / `0` hard misses / `0` soft misses. The honest blocker at the end of Session 10.8 was no longer method search; it was independent validation, because the late overlay wins that made the stack benchmark-perfect were not covered by the existing frozen holdout.
**Session 10.9 addendum (2026-04-22):** Session 10.9 spent the next hour on independent validation using fresh web evidence plus live read-only canonical snapshots. Result: the benchmark-clearing stack was **withdrawn as the trusted recommendation**. `place_alias` held up best, but the `maison_alias` benchmark win around `Ardhuy Cabotte` / `de la Cabotte` is now challenged by outside evidence pointing to a Rhone `Domaine la Cabotte` under the d'Ardhuy family and a separate Burgundy `Maison la Cabotte` line. `fils_person_alias` also failed to independently clear because the analogue controls still look historically entangled rather than cleanly separate. The next honest step is benchmark-truth and entity-history repair on those late overlay wins, not more bakeoff cycling or implementation work.
**Session 10.10 addendum (2026-04-22):** the user redirected Sprint 7 from a narrow truth-repair memo to a broader ground-truth foundation. Session 10.10 froze `ground_truth_seed_v1` under `data/sprints/identity-er/`: `422` validated pair records still mapping to today's `producer_dedup_pairs`, `71` singleton sanity records, and a seed summary showing `417` scoreable pair truths toward a new `1,000`-pair target. New hard rule: do **not** treat machine verdicts in `producer_dedup_pairs` as truth while `verdict_source` is blank. Future progress should come from audited expansion packs, starting with `s10_11_ground_truth_pack_001.md`, not from another thin benchmark-only winner claim.
**Session 10.11 addendum (2026-04-22):** Session 10.11 built the first audited expansion pack under that new truth frame: `data/sprints/identity-er/ground_truth_pack_001_v1.jsonl`, `ground_truth_pack_001_summary_v1.md`, and `pipeline/identity/build_ground_truth_pack_001_v1.py`. Pack 001 requested `72` candidate cases and landed `36` audited records, including `29` net-new scoreable additions (`9` `SAME_AS`, `8` `RELATED_BUT_DISTINCT`, `12` `NONE`) plus `3` quarantined disputes (`Ardhuy Cabotte`, `Protheau`, `Fery-Meunier`). The scoreable truth base now rises from `417` to `446`, leaving `+554` to the `1,000`-pair target. The strongest reusable evidence lanes were strict TTB shared-permit precision traps and web-backed subbrand / collaboration families; the biggest remaining blocker is still breadth, especially `SAME_AS` growth under a strict evidence bar. The next honest step is either approve Pack 001 as the new strict baseline and move to Pack 002, or explicitly reopen Pack 001 if the user wants more volume at the cost of looser evidence rules.
**Session 10.12 addendum (2026-04-22):** Session 10.12 built the second audited expansion pack under that same truth frame: `data/sprints/identity-er/ground_truth_pack_002_v1.jsonl`, `ground_truth_pack_002_summary_v1.md`, and `pipeline/identity/build_ground_truth_pack_002_v1.py`. Pack 002 requested `21` candidate cases and landed `21` audited records, all net-new scoreable additions (`11` `SAME_AS`, `5` `RELATED_BUT_DISTINCT`, `5` `NONE`), moving the scoreable truth base from `446` to `467`. Pack 002 also hardened the builder contract by switching from a name-keyed lookup to explicit producer IDs so cross-country same-name merchant-house cases can be audited without collapsing the two sides together. The strongest reusable evidence lanes were global merchant-house cross-country continuity, non-`FR` collaboration / parent-house families, and non-`FR` TTB shared-permit precision traps. The next honest step is Session 10.13: build Pack 003 under the same evidence bar, but bias it toward non-`FR` high-similarity `NONE` traps and better mid/tail balance instead of another core-heavy merchant-house batch.
...
**Session 9.8 recovery audit outcome (2026-04-21):** the remaining miss set in that layered fallback is unchanged from Session 9.6 and totals `9` disagreements, all missed merges (`6` hard, `3` soft). Production still needs at least `5` additional safe recoveries, the `5` blind-core blockers span `5` family/signature shapes, and `4` of the `9` misses sit outside the current routed-family specialist bundle. The multi-case signatures that do repeat are entangled with benchmark skip controls and prior false-merge traps. No new proof ran because any proof small enough to stay narrow would not change the production-readiness answer, and any proof large enough to matter would already be a broader redesign.

**Session 9.10 broader-method proof outcome (2026-04-21):** the broader method-bakeoff continuation survived its bounded proof. On the `29`-case trap-heavy subset, `merge_proposer_plus_veto_v1`, `expanded_layered_router_v1`, and `evidence_digest_then_judge_v1` all held `0` false merges while recovering `3-5` misses; `signature_router_v1` matched `expanded_layered_router_v1` exactly and was dropped as redundant. That proof result justified exactly one honest next step: a capped full rerun of those three survivors, not queue-building.

**Session 9.11 full-rerun outcome (2026-04-21):** the capped full `152`-case rerun is now complete. All three broader methods improved recall relative to `session9_7_layered_safety_sonnet_r2_narrow`, but none survived the frozen gates: `merge_proposer_plus_veto_v1` recovered `5` misses and reopened `9` false merges, `expanded_layered_router_v1` recovered `4` misses and reopened `5` false merges, and `evidence_digest_then_judge_v1` recovered `3` misses and reopened `5` false merges. None cleared production or fallback, while the Session 9.7 control still passes fallback with `0` false merges overall and `0.1316` full-benchmark flag rate. Queue-building remains blocked, and the broader method-class continuation no longer justifies more redesign by drift.

Immediate next session target:
- recommended next move: hold a high-level viability / next-steps discussion before any freeze closeout or new redesign. Focus on whether Loam should continue, freeze at `session9_7_layered_safety_sonnet_r2_narrow`, or shut down if trustworthy producer dedup is not achievable.
